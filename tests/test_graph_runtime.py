...
async def test_graph_routes_bgp_alert_and_creates_pending_summary(monkeypatch):
    ...
    summary = graph_runtime.summary_for(state["incident_id"])
    assert summary is not None
    assert summary["status"] == "waiting_approval"


def test_incident_memory_marks_chronic_after_four_distinct_events(monkeypatch):
    ...
    for idx in range(4):
        result = memory.correlate(f"noc-{idx}", alert)
    ...
    for _ in range(4):
        ...
        last = memory.correlate("noc", alert)
    assert last is not None
    assert last["chronic"] is True


def test_record_operator_decision_updates_summary(monkeypatch):
    memory = IncidentMemory(redis_url="")
    monkeypatch.setattr(graph_runtime, "INCIDENT_MEMORY", memory)
    memory.put_summary("incident-1", {"incident_id": "incident-1", "status": "waiting_approval", "title": "test"})

    updated = graph_runtime.record_operator_decision(
        "incident-1",
