...
def build_attention_sender(*, notifier=send_case_notification) -> AttentionSender:
    async def send(case: AtomicCaseProjection, request: AttentionRequest, intent: OutboxIntent) -> bool | SupersededCardDelivery | None:
        ...
        revision = datetime.fromisoformat(case.opened_at if request.kind == "new" else intent.created_at).timestamp()
